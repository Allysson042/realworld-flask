...
async def create_user(payload: RegisterUserRequest):
    async with get_db_connection() as db_conn:
        user = await users_handler.create_user(db_conn, payload.user)
        if not user:
            raise HTTPException(
                status_code=409,
...
async def get_current_user(user_id: str = Depends(get_required_user_id)):
    ...
    async with get_db_connection() as db_conn:
        user = await users_handler.get_user(db_conn, user_id)
        if user:
            return AuthUserResponse(
                user=AuthUser(
                    email=user.email,
...
async def update_user(
    ...
    async with get_db_connection() as db_conn:
        user = await users_handler.update_user(db_conn, user_id, payload.user)
        if not user:
            raise HTTPException(status_code=404, detail="User does not exist.")
