...
async def _get_curr_profile_by_id(db_session: AsyncSession, user_id: str) -> Profile:
    result = (
...
def _base_get_articles_query(
    *,
    ...
    curr_user_id: typ.Optional[typ.Union[str, UUID]] = None,
    ...
    articles = (
        await db_session.execute(
            _base_get_articles_query(
                curr_user_id=coerce_uuid(curr_user_id),
...
async def get_article_by_slug(
    db_session: AsyncSession, slug: str, curr_user_id: typ.Optional[str]
) -> typ.Optional[Article]:
    article = (
        await db_session.execute(
            _base_get_articles_query(slug=slug, curr_user_id=coerce_uuid(curr_user_id))
        )
    ).fetchone()
...
async def create_article(
    db_session: AsyncSession, curr_user_id: str, data: CreateArticleData
) -> Article:
    article = (
        await db_session.execute(
            satext(
            ...
            """
                ...
                author_user_id=coerce_uuid(curr_user_id),
    ...
    return await get_article_by_slug(db_session, article.slug, curr_user_id)


async def update_article(
    db_session: AsyncSession, curr_slug: str, curr_user_id: str, data: UpdateArticleData
) -> Article:
    ...
    return await get_article_by_slug(db_session, slug, curr_user_id)


async def delete_article(
    db_session: AsyncSession, slug: str, curr_user_id: str
) -> bool:
    result: typ.Any = await db_session.execute(
        satext(
            """
            ...
            AND author_user_id = :curr_user_id
            """
        ).bindparams(slug=slug, curr_user_id=coerce_uuid(curr_user_id))
    )
    return bool(result.rowcount)


async def create_article_comment(
    db_session: AsyncSession, slug: str, curr_user_id: str, data: CreateCommentData
    ...
    result = (
        await db_session.execute(
            ...
            """
                ...
                slug=slug, curr_user_id=coerce_uuid(curr_user_id), body=data.body
            )
        )
    ).fetchone()
    ...
    return True, Comment(
        ...
        body=result.body,
        author=await _get_curr_profile_by_id(db_session, curr_user_id),
    )
...
async def delete_article_comment(
    db_session: AsyncSession, slug: str, comment_id: str, curr_user_id: str
) -> bool:
    await db_session.execute(
        satext(
            """
            ...
            slug=slug,
            comment_id=coerce_uuid(comment_id),
            curr_user_id=coerce_uuid(curr_user_id),
        )
    )
    return True
...
async def add_article_favorite(
    ...
    await db_session.execute(
        satext(
            ...
            """
        ).bindparams(slug=slug, user_id=coerce_uuid(curr_user_id))
    )
    return await get_article_by_slug(db_session, slug, curr_user_id)
...
async def get_all_tags(db_session: AsyncSession) -> typ.List[str]:
    result = (await db_session.execute(satext("SELECT * from tags"))).fetchall()
