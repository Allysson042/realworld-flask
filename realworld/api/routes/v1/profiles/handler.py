...
async def get_profile(
    db_session: AsyncSession, username: str, curr_user_id: typ.Optional[str] = None
    ...
    result = (
        await db_session.execute(
            satext(
                ...
                """
            ).bindparams(username=username, curr_user_id=coerce_uuid(curr_user_id))
        )
    ).fetchone()
...
async def follow_profile(
    ...
    return await get_profile(db_session, username, curr_user_id)
